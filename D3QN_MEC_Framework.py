...
class Config:
    def __init__(self):
        ...
        self.learning_rate = 0.0005 # 降低LR，防止震荡
        self.batch_size = 128
        ...
        # [物理修正 1] 数据量减小，让无线传输有可能在 0.3s 内完成
        # 0.5 ~ 2.0 MB. 
        # 如果是 1MB, 速率 50Mbps -> 0.16s < 0.3s (可行)
        ...
        # [物理修正 2] 光纤设为瓶颈 (100Mbps)
        # 这样去云端(Offload Miss)大概率超时，逼迫AI去缓存
...
class D3QN_Network(nn.Module):
    ...
    def forward(self, state):
        # state shape: [batch, input_dim] (One-Hot encoded)
...
class D3QNAgent:
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            action = solve_knapsack(random_q, self.cfg.service_sizes, self.cfg.cache_capacity)
        else:
            with torch.no_grad():
                state_tensor = torch.FloatTensor(state_onehot).to(self.cfg.device)
                q_values = self.policy_net(state_tensor).cpu().numpy()[0]
                action = solve_knapsack(q_values, self.cfg.service_sizes, self.cfg.cache_capacity)
        return action
